...
describe('App - Maximum Coverage Tests', () => {
  ...
  describe('Final Recall Phase Complete Workflow', () => {
    it('should complete full recall workflow with all interactions', async () => {
      ...
      // Go back to recall
      const recallBtn = screen.getAllByRole('button').find((btn) => btn.textContent === 'Recall');
      if (recallBtn) {
  ...
  describe('Advanced Options Dialog Interactions', () => {
    it('should interact with all advanced options', async () => {
      ...
      if (advancedBtn) {
        ...
        // Toggle mode buttons
        const modeChips = screen
          .queryAllByRole('button')
          .filter((btn) => btn.textContent === 'Manual' || btn.textContent === 'Random');
  ...
  describe('Preset Loading and Export', () => {
    it('should attempt to load presets from dropdown', async () => {
      ...
      const presetButtons = screen
        .queryAllByRole('button')
        .filter(
          (btn) => btn.textContent?.includes('Choose preset') || btn.textContent?.includes('preset')
        );
  ...
  describe('Dark Mode Toggle in All Modes', () => {
    it('should toggle dark mode in setup, practice, and recall modes', async () => {
      ...
      await goToPractice(user);
      const darkModeBtnPractice = screen.queryByRole('button', {
        name: /switch to light mode|switch to dark mode/i,
      });
      ...
      if (recallBtn) {
        ...
        const darkModeBtnRecall = screen.queryByRole('button', {
          name: /switch to light mode|switch to dark mode/i,
        });
